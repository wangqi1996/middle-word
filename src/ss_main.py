...
from src.data.vocabulary import Vocabulary, TAG
from src.decoding.ss_beam_search import beam_search
...
def process_data(seqs_x, seqs_y):
    """
    将y根据不同的位置进行切分
    :param seqs_x:
    :param seqs_y:
    :param epoch: 不同的训练epoch, 保留的数据不同
    :param model: 会根据模型来确定 是否应该保留该样本
    :return:
    """
    # result_seqs_x = []
    # result_seqs_y = []
    ...
    #         temp_y.append(TAG)
    #         if seqs_y[:index]:
    #             temp_y.extend(y[:index])
    #         temp_x = x
    #         result_seqs_x.append(temp_x)
    #         result_seqs_y.append(temp_y)
    ...
    for x, y in zip(seqs_x, seqs_y):
        ...
        result_seqs_x.append(temp_x)
        result_seqs_y.append(temp_y)
    return result_seqs_x, result_seqs_y
...
def loss_validation(model, critic, valid_iterator):
    ...
    for batch in valid_iter:
        ...
        seqs_x, seqs_y = process_data(seqs_x, seqs_y)
        ...
        for batch in training_iter:  # batch_size
            ...
            try:
                ...
                for seqs_x_t, seqs_y_t in split_shard(seqs_x, seqs_y, split_size=training_configs['update_cycle']):
                    m_seqs_x_t, m_seqs_y_t = process_data(seqs_x_t, seqs_y_t)

                    x, y = prepare_data(m_seqs_x_t, m_seqs_y_t, cuda=GlobalNames.USE_GPU)
